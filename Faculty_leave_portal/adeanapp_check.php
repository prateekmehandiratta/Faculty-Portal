...
session_start(); 
    ...
    if($app!="associate_dean")     //forward or backward
    {
        if($cond==1)        //forward
        {
            echo "<h3>Application has been forwarded <br></h3>";
            $curr_status="pending by ".$forward;
            echo '<h3>'.$curr_status.'</h3>';
            ...
            <button class='button button1'><a href='adeanslogin.php'>Associate Dean-Portal </a></button>
            ";
        }
       
        else            //backward
        {
            echo "<h3>Application has been backwarded <br></h3>"; 
            $curr_status="please review"; 
            $sql8="UPDATE leave_application SET current_status='$curr_status',current_pos='$ap',reviewby='$h' WHERE leave_id='$leave_id'";
    ...
    else        //approve or reject
    {
        if($cond==1)    //approve
        {
            ...
            if($daysleft<$leave_days)       //rejection due to insufficient days
            {
                ...
                echo"
                <button class='button button1'><a href='home.php'>Home </a></button>
                <button class='button button1'><a href='adeanslogin.php'>Associate DeanD-Portal </a></button>
                ";
            }
            else
            {
                ...
                $sql11="INSERT into application_log(application_id,applicant_id	,applicant_post	,signer_id	,signer_post	,approved_time	,log_status)
                        VALUES('$leave_id','$aid','$ap','$uname','$h','$date','$stat')";
                mysqli_query($conn,$sql11);
                $cmnt="<h2>Congratulations leave granted!!</h2>";
                $by_whom="associate_dean";
                $sql12="INSERT into comments(app_id,comment,by_whom,accept,comment_time)
            ...
            //echo "Application has been approved <br>";
           
        }
        else            //reject
        {
               echo"
                <button class='button button1'><a href ='arejectdean.php'> Reject</a></button>
                <button class='button button1'><a href='abackwarddean.php'> Backward for Review</a></button>
               ";
        }
    }
?>
